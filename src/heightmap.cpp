...
HeightMap::HeightMap(glm::vec2 world_size, const unsigned char *data, int data_w, int data_h, unsigned char data_max) {
  world_to_map = glm::vec2((float)data_w, (float)data_h) / world_size;
  wi = data_w;
  hi = data_h;
  height_norm = 1.0f / data_max;
  this->data = data;
}

float HeightMap::get_pixel(int x, int y) {
  return height_norm * data[x+y*wi];
}
